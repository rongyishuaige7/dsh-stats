import { parseOpenAI, openRouterRule, appendObservedRule, collect } from '../scripts/collect-pricing.mjs';
...
test('collection separates reviewed official candidates and publishes only validated structured rows', async () => {
  const fetchSource = async url => url.includes('openrouter') ? { data: Array.from({ length: 10 }, (_, i) => model('vendor/new-' + i)) } : markdown;
  const result = await collect({ fetchSource, at });
  ...
  const repeat = await collect({ fetchSource, at, baseCatalog: result.catalog });
  expect(repeat.report.changes).toBe(0);
  const failed = await collect({ fetchSource: async () => { throw new Error('offline'); }, at });
  expect(failed.report.failures).toHaveLength(2);
