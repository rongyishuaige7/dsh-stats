...
const _costSummary$schema = z.object({
  status: z.enum(['exact', 'estimated', 'partial', 'unsupported']), totals: z.array(_costTotal$schema),
...
const _cost$schema = z.object({
  ...
  amount: z.number().nonnegative().nullable(), currency: z.string().nullable(), exactAmount: z.number().nonnegative(), estimatedAmount: z.number().nonnegative(), unpricedTokens: z.number().nonnegative(),
  ruleId: z.string().nullable(), sourceUrl: z.string().url().nullable(), retrievedAt: z.string().nullable(), providerId: z.string(), providerFamily: z.string(), modelCanonical: z.string(),
}).strict()
const _modelUsage$schema = z.object({
  model: z.string(), providerId: z.string(), providerFamily: z.string(), modelRaw: z.string(), modelCanonical: z.string(), accountType: z.string(), uncached: z.number().nonnegative(), output: z.number().nonnegative(),
  cacheRead: z.number().nonnegative(), cacheWrite: z.number().nonnegative(), reasoning: z.number().nonnegative(),
...
const _session$schema = z.object({
  ...
  model: z.string().nullable(), providerId: z.string(), providerFamily: z.string(), modelRaw: z.string(), modelCanonical: z.string(), accountType: z.string(), modelUsage: z.array(_modelUsage$schema), cost: _costSummary$schema, archived: z.boolean(), blank: z.boolean(),
  ...
  slotUsage: z.array(z.object({ model: z.string(), providerId: z.string(), providerFamily: z.string(), modelRaw: z.string(), modelCanonical: z.string(), accountType: z.string(), serviceTier: z.enum(['standard', 'priority']), contextTokens: z.number().nonnegative(), contextOver512k: z.boolean(), slot: z.number().int().nonnegative(), uncached: z.number().nonnegative(), output: z.number().nonnegative(), cacheRead: z.number().nonnegative(), cacheWrite: z.number().nonnegative(), reasoning: z.number().nonnegative(), cost: _cost$schema }).strict()),
...
const _providersResult$schema = z.object({ generatedAt: z.number().nonnegative(), providers: z.array(z.object({
  id: z.string(), displayName: z.string(), providerFamily: z.string(), accountMode: z.enum(['balance', 'subscription', 'unsupported']), adapter: z.string().nullable(), configured: z.boolean(), status: _providerStatus$schema, fetchedAt: z.number().nonnegative().nullable(),
}).strict()) }).strict()
const _balanceView$schema = z.object({ currency: z.string(), remaining: z.number().nonnegative(), used: z.number().nonnegative().nullable(), total: z.number().nonnegative().nullable(), toppedUp: z.number().nonnegative().nullable(), granted: z.number().nonnegative().nullable(), unlimited: z.boolean() }).strict()
const _quotaWindow$schema = z.object({ kind: z.string(), usedPercent: z.number().min(0).max(100), remainingPercent: z.number().min(0).max(100), resetsAt: z.number().nonnegative().nullable() }).strict()
const _accountResult$schema = z.object({
  generatedAt: z.number().nonnegative(), accounts: z.array(z.object({
    id: z.string(), displayName: z.string(), providerFamily: z.string(), mode: z.enum(['balance', 'subscription', 'unsupported']), adapter: z.string().nullable(), status: z.enum(['ok', 'not-configured', 'unauthorized', 'rate-limited', 'unavailable', 'invalid-response', 'blocked', 'unsupported']), stale: z.boolean(), fetchedAt: z.number().nonnegative(), lastSuccessAt: z.number().nonnegative().nullable(), errorCode: z.string().nullable(), missingCredential: z.string().nullable(), actionUrl: z.string().url().nullable(), balance: _balanceView$schema.nullable(), plan: z.string().nullable(), windows: z.array(_quotaWindow$schema),
  }).strict()), warnings: z.array(z.object({ providerId: z.string(), code: z.string(), message: z.string() }).strict()),
}).strict()
