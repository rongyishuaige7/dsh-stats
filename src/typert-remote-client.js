...
const _session$schema = z.object({
  ...
  slotUsage: z.array(z.object({ model: z.string(), slot: z.number().int().nonnegative(), uncached: z.number().nonnegative(), output: z.number().nonnegative(), cacheRead: z.number().nonnegative(), cacheWrite: z.number().nonnegative(), reasoning: z.number().nonnegative() }).strict()),
