...
function projectionRouteRows(route) {
  ...
  const rows = Array.isArray(raw) ? raw : objectRecord2(raw) ? Object.values(raw) : [];
  return rows.filter((row) => objectRecord2(row) && typeof row.model === "string" && row.model.trim() && Number.isFinite(row.time) && row.time >= 0 && (row.slot === void 0 || Number.isSafeInteger(row.slot) && row.slot >= 0));
}
...
function routeProjectionValueFromEntry(entry, sessionId, header, warnings, domainVersion) {
  ...
  const value = row.val;
  try {
    if (objectRecord2(value?.routes) && objectRecord2(value?.samples)) routeProjectionStateSchema(value);
...
function infoFromProjectionValues(values, entry) {
  ...
  const current = primary || objectRecord2(routeProjection?.current) || {};
  return {
    times,
