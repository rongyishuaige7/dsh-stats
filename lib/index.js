...
var sessionsDirCache = { at: 0, dirs: [] };
function sessionDirs(home) {
  const now = Date.now();
  if (now - sessionsDirCache.at > 5e3) {
    try {
      sessionsDirCache = { at: now, dirs: readdirSync(join(home, "sessions")) };
    } catch {
      sessionsDirCache = { at: now, dirs: [] };
    }
  }
...
function sessionInfo(home, sessionId) {
  ...
  try {
    ...
    return { times: [], model: null, usages: [], origin: null, parentSession: null };
  }
    ...
    async aggregate() {
      ...
      const processSession = (sessionId, cwdFallback) => {
        ...
        const raw = {
          ...
          uncached: usageTotals.uncachedInputTokens ?? 0,
          output: usageTotals.outputTokens ?? 0,
          cacheRead: usageTotals.cacheReadTokens ?? 0,
          cacheWrite: usageTotals.cacheWriteTokens ?? 0
        };
        return {
          id: sessionId,
          title: title ?? null,
