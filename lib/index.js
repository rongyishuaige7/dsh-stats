...
var PROJECTION_ROW_VERSIONS = Object.freeze({ sessionStats: 1, tokenUsage: 1, title: 1, sessionListMetadata: 1, statsRoute: 2 });
...
function routeProjectionView(state) {
  const routes = import_route_data.default.routeRows(state).map((row) => ({ ...row })).sort((a, b) => a.slot - b.slot || a.providerId.localeCompare(b.providerId) || String(a.model ?? "").localeCompare(String(b.model ?? "")));
  const primary = import_route_data.default.primaryRoute(routes, state.current);
  return {
    origin: state.origin,
...
function deriveSessionInfoFromEvents(rawEvents, header = null, quality = {}) {
  ...
  return {
    ...
    partial: Boolean(quality.partial) || malformedRecords > 0 || events.length === 0 && !header || seqGap || unknownSeed || header?.version !== void 0 && ![0, 1, 2].includes(header.version),
    ...
    futureVersion: header?.version !== void 0 && ![0, 1, 2].includes(header.version),
    header: header || null
  };
}
