...
function routeProjectionRoute(config, current) {
  ...
  return {
    ...
    serviceTier: import_pricing2.default.normalizeServiceTier(requestedTier || current?.serviceTier)
  };
}
...
function deriveSessionInfoFromEvents(rawEvents, header = null, quality = {}) {
  ...
  for (const ev of events) {
    ...
    if (ev.type === "session") {
      ...
      currentServiceTier = import_pricing2.default.normalizeServiceTier(config?.serviceTier || config?.service_tier);
...
function infoFromProjectionValues(values, entry) {
  ...
  const usages = routeRows.map((row) => ({
    ...
    serviceTier: import_pricing2.default.normalizeServiceTier(row.serviceTier),
...
function slotUsages(usages) {
  ...
  for (const u of usages) {
    ...
    const identity = rawIdentity(u.providerId, u.model, u.accountType, u.time);
    const serviceTier = import_pricing2.default.normalizeServiceTier(u.serviceTier);
