	...
	factory: (require) => {
		...
		var require_pricing = __commonJS({
		  "src/pricing.cjs"(exports2, module2) {
		    ...
		    function convertCostSummaryToCny2(summary, options) {
		    ...
		    function summarizeCostsCny(costs, options) {
		      return summarizeCosts2((costs || []).map(function(cost) {
		        return convertCostToCny2(cost, options);
		      }));
		    }
		    function mergeCostSummariesCny(summaries, options) {
		      return mergeCostSummaries2((summaries || []).map(function(summary) {
		        return convertCostSummaryToCny2(summary, options);
		      }));
		    }
		    ...
		    function priceUsage(usage, identityInput) {
		      ...
		      var uncertain = usage?.pricingIncomplete === true || resolved.estimatedFallback || rule.confidence === "estimated" || rule.cacheWriteDurationUnknown && tokens.cacheWrite > 0 || rule.cacheStorageUnknown && tokens.cacheWrite > 0 || rule.cacheWritePriceUnknown && tokens.cacheWrite > 0;
		    ...
		    module2.exports = {
		      ...
		      convertCostSummaryToCny: convertCostSummaryToCny2,
		      summarizeCostsCny,
		      mergeCostSummariesCny,
		      emptyCostSummary,
		      pricingCatalog,
		      tokenCounts
		    };
		...
		var require_route_data = __commonJS({
		  "src/route-data.cjs"(exports2, module2) {
		    ...
		    function routeRows(route) {
		      ...
		      if (route.routeTree !== void 0) return treeRows(route.routeTree);
		    ...
		    module2.exports = { updateRoute, routeRows, primaryRoute };
		  }
		});
		...
		var routeData = require_route_data();
		...
		function fmtCostSummary(summary) {
		  ...
		  var display2 = convertCostSummaryToCny(summary);
		  if (!display2 || !Array.isArray(display2.totals)) return "\u2014";
		  if (display2.totals.length === 0) return display2.status === "free" ? "\xA50" : "\u2014";
		  return display2.totals.map(function(total) {
		...
		function unwrapProjectionValue(value) {
		  if (isRecord(value) && Object.prototype.hasOwnProperty.call(value, "val") && (Object.prototype.hasOwnProperty.call(value, "ver") || Object.prototype.hasOwnProperty.call(value, "seq"))) return value.val;
		  return value;
		}
		function projectionValuesOf(s) {
		  ...
		  Object.keys(rows).forEach(function(key) {
		    values[key] = unwrapProjectionValue(rows[key]);
		  });
		  return values;
		}
		...
		function routeRowsOf(route) {
		  try {
		    return routeData.routeRows(route);
		  } catch {
		    return [];
		  }
		}
		function routeRowWeight(row) {
		  return nonNegativeFinite(row?.uncached) + nonNegativeFinite(row?.output) + nonNegativeFinite(row?.cacheRead) + nonNegativeFinite(row?.cacheWrite);
		}
		function compareRouteRows(a, b) {
		  return nonNegativeFinite(a?.slot) - nonNegativeFinite(b?.slot) || String(a?.providerId || "").localeCompare(String(b?.providerId || "")) || String(a?.model || "").localeCompare(String(b?.model || ""));
		}
		function projectionIdentityOf(s) {
		  var route = projectionValueOf(s, "statsRoute");
		  var current = isRecord(route?.current) ? route.current : {};
		  var rows = routeRowsOf(route).filter(function(row) {
		    return isRecord(row) && usableString(row.model);
		...
		function projectionSlotUsageOf(s, identity) {
		  ...
		  return routeRowsOf(route).filter(function(row) {
		    ...
		    var normalized = pricing.normalizeIdentity(
		      providerId,
		      modelRaw,
		      accountType,
		    ...
		    var slot = Number.isSafeInteger(row.slot) && row.slot >= 0 ? row.slot : Number.isFinite(row.time) && row.time >= 0 ? Math.floor(row.time / SLOT_MS) : null;
		    ...
		    return {
		      ...
		      contextTokens,
		      contextOver512k: contextTokens > 512e3,
		  ...
		  var cards = sorted.map(function(p) {
		    ...
		    if (isSel) {
		      ...
		      var sessRow = function(sd) {
		        ...
		        var sessionCost2 = fmtCostSummary(costDetail);
		        var titleContent = [
		          sessionTitle,
		          sd.subagent ? e("span", { className: "dss-tag", key: "subagent" }, t("w.subagentTag")) : null,
		          sd.archived ? e("span", { className: "dss-tag", key: "archived" }, t("w.archivedTag")) : null,
		          sd.quality === "partial" ? e("span", { className: "dss-tag", key: "partial", title: t("w.partialHint") }, t("w.partialTag")) : sd.quality === "stale" ? e("span", { className: "dss-tag", key: "stale", title: t("w.staleHint") }, t("w.staleTag")) : null,
		...
		module.exports.__test = {
		  ...
		  buildTimeline,
		  aggregate,
		  projectionIdentityOf,
		  projectionSlotUsageOf,
		  enrichSessionProjection,
