	...
	factory: (require) => {
		...
		function ModelRing(props) {
		  ...
		  var stops = models.map(function(m) {
		    ...
		    return { color: modelColor(m.model || "(unknown)"), from, to, label: m.model || "(unknown)", pct: (v * 100).toFixed(1), model: m };
		  });
		...
		function modelAgg(sessions) {
		  ...
		  sessions.forEach((s) => {
		    ...
		    var entries = modelTok.size ? Array.from(modelTok.entries()).map(([m, t]) => ({ model: m, uncached: t.uncached, output: t.output, cacheRead: t.cacheRead, cacheWrite: t.cacheWrite, reasoning: t.reasoning, cost: t.cost, costKnown: t.costKnown })) : [{
		    ...
		    entries.forEach((e2) => {
		      ...
		      if (!e2.costKnown) cur.costKnown = false;
		      else cur.cost += e2.cost || 0;
