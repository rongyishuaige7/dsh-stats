window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function applyDate(projects, dateKey) {
		  ...
		  return projects.map((p) => {
		    ...
		    var clipped = sessions.map(function(s) {
		      ...
		      var clippedModelUsage = (s.modelUsage || []).map(function(e2) {
		        return {
		          model: e2.model,
		          uncached: Math.round((e2.uncached || 0) * ratio),
		          output: Math.round((e2.output || 0) * ratio),
		          cacheRead: Math.round((e2.cacheRead || 0) * ratio),
		          cacheWrite: Math.round((e2.cacheWrite || 0) * ratio),
		          reasoning: Math.round((e2.reasoning || 0) * ratio)
		        };
		      });
		      var st = s.stats || {};
		  ...
		  return e(
		    ...
		    e(
		      ...
		      e(
		        ...
		        ) : tab === "timeline" ? e(TimelineView, { projects: dateProjects, timeline: viewTimeline, hidden, tt: t }) : e(TrendsView, { globals, dateGlobals, selectedDate: effectiveDate })
		      )
		    )
		  );
		}
		...
		function modelAgg(sessions) {
		  ...
		  sessions.forEach((s) => {
		    var st = s.stats || {};
		    var entries = s.modelUsage && s.modelUsage.length ? s.modelUsage : [{
