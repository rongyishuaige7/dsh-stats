window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function sortValue(p, key) {
		  switch (key) {
		    case "cost": {
		      var cost = projectCostSummary(p);
		      return cost.totals.length ? cost.totals[0].amount : -1;
		    }
		    case "input":
		      ...
		      accounts.length > 1 ? e(
		        ...
		        e(
		          ...
		          accounts.map(function(item) {
		            return e("option", { key: item.id, value: item.id }, item.displayName + " \xB7 " + t("balance.status." + (item.stale ? "stale" : item.status)));
		          })
		        )
		      ) : null
		    ),
		      ...
		      e(
		        ...
		        tab === "balance" ? e(BalanceView, { data: balanceData, state: balanceState, remote: balanceRemote, t }) : e(
		          ...
		          tab === "overview" ? e(
		            ...
		            visibleProjects.length === 0 ? e("div", { className: "dss-empty" }, t("empty")) : e(ProjectsTable, { projects: statProjects, hidden, selected, t, dayMode: effectiveDate != null, onSelect: (id) => setSelected((s) => s === id ? null : id) })
		          ) : tab === "timeline" ? e(TimelineView, { projects: dateProjects, timeline: viewTimeline, hidden, dayMode: effectiveDate != null, tt: t }) : e(TrendsView, { globals, dateGlobals, selectedDate: effectiveDate, t })
		        )
		      )
		    )
		  );
		...
		function ModelRing(props) {
		  ...
		  var stops = models.map(function(m) {
		    ...
		    return { color: modelColor(m.key || m.displayName || m.model || "(unknown)"), from, to, label: m.displayName || modelDisplayName(m), pct: (v * 100).toFixed(1), model: m };
		  });
